...
const PARKED_CRYPTO_ASSETS=[
 {id:1,symbol:"USDT",network:"TRC20",deposit_address:"THnBWVJRDbQpTDmtNSegYvpYjEDr4PgYQ3",memo_tag:"",active:true,sort_order:0},
 {id:2,symbol:"TRX",network:"TRC20",deposit_address:"THnBWVJRDbQpTDmtNSegYvpYjEDr4PgYQ3",memo_tag:"",active:true,sort_order:1},
 {id:3,symbol:"ETH",network:"Ethereum",deposit_address:"0xbd1255bb4d2d6b4d4817830dae8fe43cd3202ba0",memo_tag:"",active:true,sort_order:2}
];
let memoryCryptoAssets=PARKED_CRYPTO_ASSETS.map(x=>({...x}));
...
async function initDb(){
 ...
 CREATE TABLE IF NOT EXISTS crypto_assets(id BIGSERIAL PRIMARY KEY,symbol TEXT NOT NULL,network TEXT NOT NULL DEFAULT '',deposit_address TEXT NOT NULL,memo_tag TEXT NOT NULL DEFAULT '',active BOOLEAN NOT NULL DEFAULT TRUE,sort_order INT NOT NULL DEFAULT 0,updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW());
...
async function currentMemberFromReq(req){const token=cookieValue(req,"majorcoin_member_session");const x=memberSessions.get(token);if(!x||x.exp<Date.now()){if(token)memberSessions.delete(token);return null;}x.exp=Date.now()+7*24*60*60*1000;if(x.sample)return {id:0,login_id:"tttt",name:"샘플회원",phone:"",email:"",point_balance:0,sample:true};if(!pool)return null;const r=await pool.query("SELECT id,login_id,name,phone,email,point_balance FROM members WHERE id=$1",[x.memberId]);return r.rows[0]||null;}
...
app.post("/api/auth/login",async(req,res)=>{const {loginId,password}=req.body||{};if(String(loginId)==="tttt"&&String(password)==="3333"){const token=crypto.randomBytes(32).toString("hex");memberSessions.set(token,{sample:true,exp:Date.now()+7*24*60*60*1000});res.setHeader("Set-Cookie",`majorcoin_member_session=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=604800`);return res.json({ok:true,user:{id:0,login_id:"tttt",name:"샘플회원",phone:"",email:"",point_balance:0,sample:true}});}if(!pool)return res.status(503).json({ok:false,error:"DB 연결 전에는 샘플계정 tttt / 3333 으로 로그인해주세요."});const r=await pool.query("SELECT * FROM members WHERE login_id=$1",[loginId]);if(!r.rows.length||!verifyPassword(password,r.rows[0].password_hash))return res.status(401).json({ok:false,error:"아이디 또는 비밀번호가 올바르지 않습니다."});const token=crypto.randomBytes(32).toString("hex");memberSessions.set(token,{memberId:r.rows[0].id,exp:Date.now()+7*24*60*60*1000});res.setHeader("Set-Cookie",`majorcoin_member_session=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=604800`);res.json({ok:true,user:{id:r.rows[0].id,login_id:r.rows[0].login_id,name:r.rows[0].name,phone:r.rows[0].phone,email:r.rows[0].email,point_balance:r.rows[0].point_balance}});});
app.get("/api/auth/me",async(req,res)=>{const user=await currentMemberFromReq(req);res.json({ok:true,user});});
...
app.post("/api/points/recharge-request",needDb,async(req,res)=>{const user=await currentMemberFromReq(req);if(!user)return res.status(401).json({ok:false,error:"로그인이 필요합니다."});const amount=Number(req.body?.amount),method=String(req.body?.method||"");const bonuses={100000:0,300000:30000,500000:50000,1000000:100000};if(!(amount in bonuses)||!['COIN','BANK'].includes(method))return res.status(400).json({ok:false,error:"충전 금액 또는 방법이 올바르지 않습니다."});const rid=`RC-${Date.now()}-${crypto.randomBytes(3).toString('hex')}`;await pool.query("INSERT INTO recharge_requests(recharge_id,member_id,method,requested_amount,bonus_amount) VALUES($1,$2,$3,$4,$5)",[rid,user.id,method,amount,bonuses[amount]]);res.json({ok:true,rechargeId:rid,amount,bonus:bonuses[amount],message:method==='COIN'?'코인 입금주소/API 연동 전 단계입니다. 관리자 설정 후 자동입금 확인을 연결할 수 있습니다.':'지정 계좌 입금 확인 후 포인트가 적립됩니다.'});});
...
async function getCryptoAssets(){
 if(pool){try{const r=await pool.query("SELECT id,symbol,network,deposit_address,memo_tag,active,sort_order FROM crypto_assets ORDER BY sort_order,id");if(r.rows.length)return r.rows;return PARKED_CRYPTO_ASSETS.map(x=>({...x}));}catch(e){console.error('crypto assets',e.message)}}
 return memoryCryptoAssets.length?memoryCryptoAssets:PARKED_CRYPTO_ASSETS.map(x=>({...x}));
}
app.get('/api/crypto-assets',async(req,res)=>{const rows=(await getCryptoAssets()).filter(x=>x.active&&String(x.deposit_address||'').trim());res.json({ok:true,assets:rows.map(x=>({symbol:String(x.symbol).toUpperCase(),network:x.network||'',memoTag:x.memo_tag||''}))})});
app.get('/api/crypto-rates',async(req,res)=>{try{const assets=(await getCryptoAssets()).filter(x=>x.active&&String(x.deposit_address||'').trim());const symbols=[...new Set(assets.map(x=>String(x.symbol||'').toUpperCase()).filter(x=>/^[A-Z0-9]{2,12}$/.test(x)))];if(!symbols.length)return res.json({ok:true,rates:{},updatedAt:Date.now()});const markets=symbols.map(x=>'KRW-'+x).join(',');const r=await fetch('https://api.bithumb.com/v1/ticker?markets='+encodeURIComponent(markets),{headers:{accept:'application/json'}});if(!r.ok)throw new Error('Bithumb ticker HTTP '+r.status);const data=await r.json();const rates={};for(const x of Array.isArray(data)?data:[]){const sym=String(x.market||'').replace(/^KRW-/,'');const price=Number(x.trade_price);if(price>0)rates[sym]=price}res.setHeader('Cache-Control','public, max-age=20');res.json({ok:true,rates,updatedAt:Date.now()})}catch(e){res.status(502).json({ok:false,error:e.message,rates:{}})}});
app.get('/api/admin/crypto-assets',needAdmin,async(req,res)=>{res.json({ok:true,assets:await getCryptoAssets(),api:{bithumbKey:!!process.env.BITHUMB_API_KEY,bithumbSecret:!!process.env.BITHUMB_API_SECRET}})});
app.post('/api/admin/crypto-assets',needAdmin,async(req,res)=>{try{const rows=(Array.isArray(req.body?.assets)?req.body.assets:[]).map((x,i)=>({symbol:String(x.symbol||'').trim().toUpperCase(),network:String(x.network||'').trim(),deposit_address:String(x.deposit_address||'').trim(),memo_tag:String(x.memo_tag||'').trim(),active:x.active!==false,sort_order:i})).filter(x=>x.symbol&&x.deposit_address);if(pool){const c=await pool.connect();try{await c.query('BEGIN');await c.query('DELETE FROM crypto_assets');for(const x of rows)await c.query('INSERT INTO crypto_assets(symbol,network,deposit_address,memo_tag,active,sort_order) VALUES($1,$2,$3,$4,$5,$6)',[x.symbol,x.network,x.deposit_address,x.memo_tag,x.active,x.sort_order]);await c.query('COMMIT')}catch(e){await c.query('ROLLBACK');throw e}finally{c.release()}}else memoryCryptoAssets=rows.map((x,i)=>({id:i+1,...x}));res.json({ok:true,count:rows.length,persistent:!!pool})}catch(e){res.status(500).json({ok:false,error:e.message})}});

app.get("/api/store-settings",async(req,res)=>{let settings={slideYoutube:"",shortsUrl:"",slidesJson:"[]",slideTime:"5",homeSectionsJson:"[]",footerText:"회사명: 메이저코인 쇼핑몰\n고객센터: 02-000-8282",copyright:"Copyright © 2026 메이저코인 쇼핑몰. All rights reserved.",terms:"제1조(목적)\n본 약관은 메이저코인 쇼핑몰의 서비스 이용 조건과 절차를 정합니다.\n\n제2조(주문 및 결제)\n상품 주문, 결제, 배송 및 취소는 화면에 표시된 절차와 관련 법령에 따릅니다.\n\n제3조(환불)\n환불 및 교환은 상품 특성과 관련 법령, 판매자가 고지한 기준에 따릅니다.",naverSiteVerification:"",googleSiteVerification:"",privacy:"메이저코인 쇼핑몰은 주문 처리와 배송을 위해 필요한 범위에서 개인정보를 처리합니다.\n\n수집 항목: 주문자/수령인 이름, 연락처, 배송지, 주문 및 결제 식별정보\n이용 목적: 주문 확인, 결제 확인, 배송, 고객 문의 처리\n보유 기간: 관련 법령 및 운영상 필요한 기간 동안 보관 후 파기합니다.\n\n이 기본 문안은 운영자가 실제 사업 내용과 적용 법령에 맞게 검토·수정해야 합니다."};if(pool){try{const r=await pool.query("SELECT key,value FROM store_settings");for(const x of r.rows)settings[x.key]=x.value}catch{}}res.json({ok:true,settings});});
