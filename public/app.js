let products = [];
let cryptoRates = {}; let activeCryptoAssets = [];
function coinDecimals(sym){return ["BTC","ETH"].includes(sym)?6:["USDT","USDC"].includes(sym)?2:4}
function cryptoPriceHtml(krw){const rows=activeCryptoAssets.map(x=>{const s=String(x.symbol||"").toUpperCase(),rate=Number(cryptoRates[s]);if(!rate)return "";return `<span>${s} ${(Number(krw)/rate).toFixed(coinDecimals(s))}</span>`}).filter(Boolean);return rows.length?`<div class="crypto-price-list">${rows.join("")}</div>`:""}
async function loadCryptoRates(){try{const [a,r]=await Promise.all([fetch("/api/crypto-assets").then(x=>x.json()),fetch("/api/crypto-rates").then(x=>x.json())]);activeCryptoAssets=a.assets||[];cryptoRates=r.rates||{};renderProducts();}catch(e){console.warn("crypto rates",e)}}
let storeSettings={slideYoutube:"",shortsUrl:"",footerText:"회사명: 메이저코인 쇼핑몰\n고객센터: 02-000-8282",copyright:"Copyright © 2026 메이저코인 쇼핑몰. All rights reserved.",terms:"",privacy:""};
...
function openProductDetail(id){
 ...
 <h2>${p.name}</h2><p>${p.detail||p.desc||""}</p><b class="detail-price">${Math.round(Number(p.price)).toLocaleString("ko-KR")}원</b>${cryptoPriceHtml(p.price)}
